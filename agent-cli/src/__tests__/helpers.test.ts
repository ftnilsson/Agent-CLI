...
describe("composeSlimAgentFile", () => {
  const HEADER = "<!-- Auto-generated by agent-cli. Do not edit manually. -->";

  it("includes generation header", () => {
    const result = composeSlimAgentFile([], []);
    assert.ok(result.includes(HEADER));
  });

  it("renders agent refs under ## Agent Instructions", () => {
    const result = composeSlimAgentFile(
      [{ label: "agents/nextjs", filePath: ".github/agents/agents-nextjs.md" }],
      [],
    );
    assert.ok(result.includes("## Agent Instructions"));
    assert.ok(result.includes("[agents/nextjs](.github/agents/agents-nextjs.md)"));
  });

  it("renders skill refs under ## Skills grouped by category", () => {
    const result = composeSlimAgentFile([], [
      { label: "development/git", filePath: ".github/skills/02-git/skill.md" },
      { label: "development/architecture", filePath: ".github/skills/01-arch/skill.md" },
    ]);
    assert.ok(result.includes("## Skills"));
    assert.ok(result.includes("### development"));
    assert.ok(result.includes("[development/git](.github/skills/02-git/skill.md)"));
    assert.ok(result.includes("[development/architecture](.github/skills/01-arch/skill.md)"));
  });

  it("groups skills from different categories into separate subsections", () => {
    const result = composeSlimAgentFile([], [
      { label: "development/git", filePath: ".github/skills/git/skill.md" },
      { label: "frontend/components", filePath: ".github/skills/components/skill.md" },
    ]);
    assert.ok(result.includes("### development"));
    assert.ok(result.includes("### frontend"));
  });

  it("renders local overrides inline before agent and skill sections", () => {
    const overrides = "## My Project Rules\n\n- Never skip tests.";
    const result = composeSlimAgentFile([], [], overrides);
    const overridePos = result.indexOf("## My Project Rules");
    const skillsPos = result.indexOf("## Skills");
    const agentsPos = result.indexOf("## Agent Instructions");
    assert.ok(overridePos > -1);
    // overrides should appear before both sections (or before the end if sections are absent)
    if (skillsPos > -1) assert.ok(overridePos < skillsPos);
    if (agentsPos > -1) assert.ok(overridePos < agentsPos);
  });

  it("omits ## Agent Instructions section when agentRefs is empty", () => {
    const result = composeSlimAgentFile([], [
      { label: "development/git", filePath: ".github/skills/git/skill.md" },
    ]);
    assert.ok(!result.includes("## Agent Instructions"));
  ...
  it("omits ## Skills section when skillRefs is empty", () => {
    const result = composeSlimAgentFile(
      [{ label: "agents/nextjs", filePath: ".github/agents/agents-nextjs.md" }],
      [],
    );
  ...
  it("produces output under 200 lines for a full install scenario", () => {
    const agentRefs = [
      { label: "agents/nextjs", filePath: ".github/agents/agents-nextjs.md" },
      { label: "agents/nestjs", filePath: ".github/agents/agents-nestjs.md" },
    ];
    const skillRefs = Array.from({ length: 15 }, (_, i) => ({
      label: `development/skill-${i}`,
      filePath: `.github/skills/skill-${i}/skill.md`,
    }));
    const result = composeSlimAgentFile(agentRefs, skillRefs);
